from sim_pulse_deepseek import ParticleSimulator
import numpy as np
...
def process(KEc, L, PHI):
    try:
        ...
        p1_r, p1_t, p1_wtot = p1.run_simulation()
        
        df = pd.DataFrame(columns=["time(sec)", "x","y","z","wtot"])
        df["time(sec)"] = p1_t
        df["x"] = [r[0] for r in p1_r] 
        df["y"] = [r[1] for r in p1_r] 
        df["z"] = [r[2] for r in p1_r] 
        df["wtot"] = p1_wtot
...
if __name__ == '__main__':
    ...
    def save_batch_to_npy(batch, batch_id):
        try:
            ...
            save_path = f"./{result_name}/results_batch/batch_{batch_id}.npy"
            if not os.path.exists(f'./{result_name}/results_batch'):
                os.makedirs(f'./{result_name}/results_batch')
    ...
    with multiprocessing.Manager() as manager:
        ...
        target_count = 100000 # 模拟的粒子总数
        batch_size = 5000 # batch大小
